...
//     printf("%02d - %s\n", planet_number, PLANET_NAMES[planet_number].c_str());
...
//     printf("%f, %f, %f\n", *(float*)&ratchet_x, *(float*)&ratchet_y, *(float*)&ratchet_z);
//     return 0;
// }
...
//         std::cerr << "Cannot display message because the game is currently in a paused state";
//         return -1;
//     }

//     std::optional<uint8_t*> possible_range = ipc_read_range(ipc, (uint32_t)0x0, (uint32_t)0x2000000);
//     if(!possible_range.has_value()) {return -1;}

//     std::optional<uint32_t> search = find_string(possible_range.value(), 0x2000000, "You got a skill point!\0\0G");
//     if(!search.has_value()) {return -1;}
//     uint32_t msg_address = search.value();
...
//     // For some reason the Version call only works after PCSX2 starts running a game. 
//     printf("Platform: %s\n", ipc->Version());

//     // These calls also only work when PCSX2 is running a game but it makes more sense for these calls.
//     printf("Game: %s\n", ipc->GetGameTitle());
//     printf("ID: %s, Version: %s, UUID: %s\n", ipc->GetGameID(), ipc->GetGameVersion(), ipc->GetGameUUID());

//     return 0;
// }

// std::optional<uint32_t> get_segment_table_address(std::shared_ptr<PCSX2> ipc) {
//     const uint8_t BEGINNING_OF_TABLE[8] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0x00};
//     uint32_t* segment_pointer_table = nullptr;
...
//     std::cerr << "Could not find memory segment pointer table. You may not be on a planet.\n";
//     return std::nullopt;
// }

// int print_memory_segments() {
//     std::shared_ptr<PCSX2> ipc = std::make_shared<PCSX2>();
//     std::optional<uint32_t> segment_table_address = get_segment_table_address(ipc);
//     if(!segment_table_address.has_value()) {return -1;}

//     std::optional<uint8_t*> range = ipc_read_range(ipc, segment_table_address.value(), segment_table_address.value() + sizeof(uint32_t) * RAC2_SEGMENT_COUNT);
//     if(!range.has_value()) {return -1;}
//     uint32_t* segments = (uint32_t*)range.value();
    
//     for(int i = 0; i < RAC2_SEGMENT_COUNT; i++) {
//         printf("0x%08X - %s\n", segments[i], Rac2::RAC2_SEGMENT_NAMES[i].c_str());
//     }
...
// int moby_info(uint32_t moby_segment_start, uint32_t moby_segment_end, int uid) {
//     std::shared_ptr<PCSX2> ipc = std::make_shared<PCSX2>();

//     uint32_t moby_address = moby_segment_start + uid * MOBY_SIZE;
//     if(moby_address > moby_segment_end) {
//         std::cerr << "Invalid Moby ID: The requested moby id was too high.\n";
//         return -1;
//     }
...
//     // I don't want to talk about it-----------------V------------V------------V
//     printf("Position(0x10): X=%f, Y=%f, Z=%f\n", *(float*)&x, *(float*)&y, *(float*)&z);
...
//     printf("Moby Class Address(0x24): 0x%08X\n", ipc->Read<uint32_t>(moby_address + 0x24));
//     printf("Chain Address(0x28): 0x%08X\n", ipc->Read<uint32_t>(moby_address + 0x28));
    
//     uint32_t scale = ipc->Read<uint32_t>(moby_address + 0x2C);
//     printf("Scale(0x2C): %f\n", *(float*)&scale);
//     printf("Drawn(0x31): %d\n", ipc->Read<uint8_t>(moby_address + 0x31));
//     printf("Draw Distance(0x32): %d\n", ipc->Read<uint16_t>(moby_address + 0x32));
//     std::bitset<16> flags1(ipc->Read<uint16_t>(moby_address + 0x34));
//     std::cout << "Flags 1(0x34): 0b" << flags1 << "\n";
//     std::bitset<16> flags2(ipc->Read<uint16_t>(moby_address + 0x36));
//     std::cout << "Flags 2(0x36): 0b" << flags2 << "\n";
//     printf("Lighting?(0x38): %d\n", ipc->Read<uint32_t>(moby_address + 0x38));
//     printf("Color(0x3C): R=%d, G=%d, B=%d, Shine=%d\n", ipc->Read<uint8_t>(moby_address + 0x3C), ipc->Read<uint8_t>(moby_address + 0x3D), 
//                                                         ipc->Read<uint8_t>(moby_address + 0x3E), ipc->Read<uint8_t>(moby_address + 0x3F));

//     printf("Update Function Address(0x64): 0x%08X\n", ipc->Read<uint32_t>(moby_address + 0x64));
//     printf("pVars Address(0x68): 0x%08X\n", ipc->Read<uint32_t>(moby_address + 0x68));
//     printf("collData Address(0x98): 0x%08X\n", ipc->Read<uint32_t>(moby_address + 0x98));
//     printf("Collision Counter(0xA0): %d\n", ipc->Read<uint32_t>(moby_address + 0xA0));
//     printf("oClass(0xAA): %d\n", ipc->Read<uint16_t>(moby_address + 0xAA));
//     printf("UID(0xB2): %d\n", ipc->Read<uint16_t>(moby_address + 0xB2));

//     return 0;
// }

// std::optional<uint8_t*> ipc_read_range(std::shared_ptr<PCSX2> ipc, uint32_t start, uint32_t end) {
...
//     // Since we read to ee_memory on 8 bytes chunks, add some buffer to the end so the last read doesn't overrun. 
//     uint8_t* ee_memory = new uint8_t[memory_size + memory_size % 8];
...
//             // We read 64 bits at a time to reduce the amount of IPC reads that we need to do.
//             ipc->Read<uint64_t, true>(start + offset);
//         }
//         auto resr = ipc->FinalizeBatch();
//         ipc->SendCommand(resr);

//         for (int j = 0; j < bytes_per_batch / 8; j++) {
//             // Since the reply is in 64 bit chunks but we want to store it in an array of bytes, we create
//             // a 64 bit pointer to the beinning of the byte array and copy our 64 bit value into that.
//             uint64_t* ee_mem_64 = (uint64_t*)ee_memory;
//             ee_mem_64[(i * (bytes_per_batch / 8) + j)] = ipc->GetReply<PCSX2::MsgRead64>(resr, j);
//         }     
//     }
    
//     // Read remaining partial batch
//     //printf("\nAppending remainder batch; size: %d\n", remainder_batch_size);        
//     ipc->InitializeBatch();
//     for (int j = 0; j < remainder_batch_size; j += 8) {
...
//         // a 64 bit pointer to the beinning of the byte array and copy our 64 bit value into that.
//         uint64_t* ee_mem_64 = (uint64_t*)ee_memory;
//         ee_mem_64[(full_batches * (bytes_per_batch / 8) + j)] = ipc->GetReply<PCSX2::MsgRead64>(resr, j);
//     }  
//     return std::make_optional(ee_memory);
// }

// std::optional<uint32_t> find_byte_sequence(uint8_t* memory, size_t memory_size, const uint8_t* sequence, size_t seq_length) {
...
// std::optional<uint32_t> find_string(uint8_t* memory, size_t memory_size, std::string str) {
//     return find_byte_sequence(memory, memory_size, (uint8_t*)str.c_str(), str.length());
// }